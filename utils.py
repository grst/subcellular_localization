...
from lasagne.layers import MergeLayer, Layer, InputLayer, DenseLayer, helper, \
    Gate
...
def iterate_minibatches(inputs, targets, masks, batchsize, shuffle=False,
    ...
    len_seq = np.apply_along_axis(np.bincount, 1, masks.astype(np.int32))[:,
              -1]
    ...
    for start_idx in idx_list:
        if start_idx == last_idx:
            ...
            excerpt = np.concatenate(
                (indices[start_idx:start_idx + batchsize], B))
        else:
...
class DropoutSeqPosLayer(Layer):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        if deterministic or self.p == 0:
            ...
            mask = self._srng.binomial(input_shape[:2], p=retain_prob,
                                       dtype=input.dtype)
...
class LSTMAttentionDecodeFeedbackLayer(MergeLayer):
    ...
    def __init__(self, incoming,
        ...
        self.W_weightedhid_to_ingate = self.add_param(
            W_hid_to_ingate, (num_inputs, num_units),
            name="W_weightedhid_to_ingate")
        ...
        self.W_weightedhid_to_cell = self.add_param(
            W_hid_to_cell, (num_inputs, num_units),
            name="W_weightedhid_to_cell")

        self.W_weightedhid_to_outgate = self.add_param(
            W_hid_to_outgate, (num_inputs, num_units),
            name="W_weightedhid_to_outgate")
        ...
        self.W_weightedhid_stacked = T.concatenate(
            [self.W_weightedhid_to_ingate, self.W_weightedhid_to_forgetgate,
             self.W_weightedhid_to_cell, self.W_weightedhid_to_outgate],
            axis=1)
        ...
        self.U_align = self.add_param(U_align,
                                      (num_inputs, self.aln_num_units),
    ...
    def get_params(self, **tags):
        ...
        params = super(LSTMAttentionDecodeFeedbackLayer, self).get_params(
            **tags)
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        alpha_out = alpha_out.dimshuffle(1, 0,
                                         2)  # (BS, n_decodesteps, encode_seqlen)
