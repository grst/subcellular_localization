...
def neural_network(batch_size, n_hid, n_feat, n_class, lr, drop_per, drop_hid,
    ...
    target_var = T.matrix('targets', dtype='int64')
    mask_var = T.matrix('masks')

    # Input layer, holds the shape of the data
    l_in = lasagne.layers.InputLayer(shape=(batch_size, None, n_feat),
                                     input_var=input_var)
    ...
    l_conv_a = lasagne.layers.Conv1DLayer(l_shu, num_filters=n_filt,
                                          pad='same', stride=1, W=w_inits,
                                          filter_size=f_size_a,
                                          nonlinearity=lasagne.nonlinearities.rectify)
    l_conv_b = lasagne.layers.Conv1DLayer(l_shu, num_filters=n_filt,
                                          pad='same', stride=1, W=w_inits,
                                          filter_size=f_size_b,
                                          nonlinearity=lasagne.nonlinearities.rectify)
    l_conv_c = lasagne.layers.Conv1DLayer(l_shu, num_filters=n_filt,
                                          pad='same', stride=1, W=w_inits,
                                          filter_size=f_size_c,
                                          nonlinearity=lasagne.nonlinearities.rectify)
    l_conv_d = lasagne.layers.Conv1DLayer(l_shu, num_filters=n_filt,
                                          pad='same', stride=1, W=w_inits,
                                          filter_size=f_size_d,
                                          nonlinearity=lasagne.nonlinearities.rectify)
    l_conv_e = lasagne.layers.Conv1DLayer(l_shu, num_filters=n_filt,
                                          pad='same', stride=1, W=w_inits,
                                          filter_size=f_size_e,
                                          nonlinearity=lasagne.nonlinearities.rectify)
    l_conv_f = lasagne.layers.Conv1DLayer(l_shu, num_filters=n_filt,
                                          pad='same', stride=1, W=w_inits,
                                          filter_size=f_size_f,
                                          nonlinearity=lasagne.nonlinearities.rectify)

    # Concatenate all CNN layers
    l_conc = lasagne.layers.ConcatLayer(
        [l_conv_a, l_conv_b, l_conv_c, l_conv_d, l_conv_e, l_conv_f], axis=1)

    # Second CNN layer
    l_conv_final = lasagne.layers.Conv1DLayer(l_conc, num_filters=64,
                                              pad='same', stride=1,
                                              filter_size=f_size_b,
                                              nonlinearity=lasagne.nonlinearities.rectify)
    ...
    l_fwd = lasagne.layers.LSTMLayer(l_indrop, num_units=n_hid, name='LSTMFwd',
                                     mask_input=l_mask,
                                     cell_init=lasagne.init.Orthogonal(),
                                     hid_init=lasagne.init.Orthogonal(),
                                     nonlinearity=lasagne.nonlinearities.tanh,
                                     grad_clipping=2)
    l_bck = lasagne.layers.LSTMLayer(l_indrop, num_units=n_hid, name='LSTMBck',
                                     mask_input=l_mask,
                                     cell_init=lasagne.init.Orthogonal(),
                                     hid_init=lasagne.init.Orthogonal(),
                                     backwards=True,
    ...
    # Attention mechanism
    l_dec = LSTMAttentionDecodeFeedbackLayer(l_conc_lstm, mask_input=l_mask,
                                             num_units=n_hid * 2,
                                             aln_num_units=n_hid,
                                             n_decodesteps=10,
                                             name='LSTMAttention')
    ...
    l_dense = lasagne.layers.DenseLayer(
        lasagne.layers.dropout(l_last_hid, p=drop_hid), name="Dense",
        num_units=n_hid * 2, W=w_inits,
        nonlinearity=lasagne.nonlinearities.rectify)

    # Softmax output layer
    l_out = lasagne.layers.DenseLayer(
        lasagne.layers.dropout(l_dense, p=drop_hid), num_units=n_class,
        name="Output", W=w_inits, nonlinearity=lasagne.nonlinearities.sigmoid)
    ...
    # Loss function
    t_test_loss = lasagne.objectives.categorical_crossentropy(test_prediction,
                                                              target_var)
